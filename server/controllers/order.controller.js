...
const evaluateCoupon = async (code, userId) => {
  ...
  if (!coupon || !coupon.active) {
    throw new AppError('Cupom invalido ou expirado', 400);
  }

  const now = new Date();
  if (coupon.startsAt && now < coupon.startsAt) {
    throw new AppError('Cupom ainda nao esta disponivel', 400);
  }
  ...
  try {
    ...
    const order = await prisma.purchase.create({
      data: {
        ...
        notes: notes ?? null,
        metadata: safeJsonStringify(metadataPayload, '{}'),
        items: {
          create: cartItems.map((item) => ({
            ...
            deliveryData: safeJsonStringify(
...
export const cancelOrder = async (req, res, next) => {
  try {
    ...
    if (!['PENDING', 'AWAITING_CONFIRMATION'].includes(order.status)) {
      throw new AppError('Pedido nao pode ser cancelado neste status', 400);
    }

    const existingMetadata = safeJsonParse(order.metadata, {});
